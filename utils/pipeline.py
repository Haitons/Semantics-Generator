...
def generate_batch(model, dataloader, voc, context_voc, tau, length, device, save, strategy="Greedy"):
    model.eval()
    ...
    for inp in tqdm(dataloader, desc='Generate definitions.', leave=False):
        ...
        with torch.no_grad():
            ...
            if strategy == "Greedy":
                word_weights = torch.argmax(output, dim=1)
            elif strategy == "Multinomial":
                word_weights = F.softmax(
                    output / tau, dim=1
                ).multinomial(num_samples=1)
            data["seq"] = word_weights.view(1, batch_size)
            keep_decoding = [1] * batch_size
            for k in range(len(keep_decoding)):
                word_id = word_weights[k].item()
                if word_id != constants.EOS_IDX:
                    decoded_words[k].append(voc.decode(word_id))
                else:
                    keep_decoding[k] = 0
            ...
            for k in range(len(decoded_words)):
                ...
                ret.append((voc.decode(word[k]), data["context_text"][k], decoded_words[k]))
        for i in range(len(ret)):
            defsave.write("Word:" + ret[i][0] + "\n")
            defsave.write("Context:")
            defsave.write(" ".join(ret[i][1]) + "\n")
            defsave.write("Definition:")
            defsave.write(" ".join(ret[i][2]) + "\n")
    ...
    for inp in tqdm(dataloader, desc='Generate definitions.', leave=False):
        ...
        with torch.no_grad():
            for i in range(length):
                ...
                if word_weights == constants.EOS_IDX:
                    break
                else:
                    data["seq"] = word_weights.view(1, batch_size)
                    word = word_weights.item()
