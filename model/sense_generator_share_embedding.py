...
class SenseGenerator_ShareEmbedding(nn.Module):
    def __init__(self, params):
        super(SenseGenerator_ShareEmbedding, self).__init__()
        ...
        self.task2 = GruDecoder(params, self.embedding_dim, self.cond_size)
        self.init_weights()
    ...
    def forward(self, inputs, init_hidden1, init_hidden2, return_h=False):
        ...
        if init_hidden1 is None:
            if self.ws == "word":
                init_hidden1 = self.init_hidden(
        ...
        if init_hidden2 is None:
            if self.ws == "word":
                init_hidden2 = self.init_hidden(
                    word_emb, batch_size, self.params["nlayers"], self.params["hidim"]
                )
            elif self.ws == "word+contexts":
                init_hidden2 = self.init_hidden(
