...
class SenseGenerator(nn.Module):
    def __init__(self, params):
        super(SenseGenerator, self).__init__()
        self.params = params
        ...
        if self.params["pretrain"]:
            assert self.params["use_input"] + self.params["use_input_adaptive"] + self.params["use_input_attention"] + \
                   self.params["use_hidden"] + self.params["use_gated"] + \
                   self.params["use_ch"] + self.params["use_elmo"] == 0, "Pretrain LM without any condition!"
            self.cond_size = None
        else:
            ...
            assert self.use_input + self.use_hidden + \
                   self.use_gated + self.is_ada + self.is_attn <= 1, "Too many conditionings used"
        ...
        self.gru = GruDecoder(params, self.embedding_dim, self.cond_size)
    ...
    def forward(self, inputs, init_hidden, return_h=False):
        ...
        task_output = self.gru(data, hidden, return_h=return_h)
        outputs = task_output[0]
        hidden = task_output[1]
        decoded = self.decoder(self.drop(outputs))
        if return_h:
            rnn_hs = task_output[2]
            dropped_rnn_hs = task_output[3]
