...
class SenseGenerator(nn.Module):
    def __init__(self, params):
        ...
        self.task1 = params["task1"]
        self.model1 = GruDecoder(params, self.embedding_dim, self.cond_size)
        self.model2 = GruDecoder(params, self.embedding_dim + self.params["hidim"],
                                self.cond_size + self.params["hidim"])
        self.init_weights()

    def forward(self, inputs, init_hidden, main_task, return_h=False):
        seq = inputs["seq"]
        ...
        hidden=init_hidden
        task_output = self.model1(data, hidden,return_h)
        outputs = task_output[0]
        # hidden2 = hidden
        if main_task:
            outputs = outputs.view(data['seq_emb'].size(0), data['seq_emb'].size(1), -1)
            data['seq_emb'] = torch.cat([data['seq_emb'], outputs], -1)
            # task_output = self.model2(data, hidden2)
            task_output = self.model2(data, hidden,return_h)
            outputs = task_output[0]
