...
from utils.pipeline_multi_taslk import train_epoch, test, train_shunxu
...
parser.add_argument(
    "----multi_task", action="store_true",
    help="whether to update embedding matrix or not"
)
parser.add_argument(
    '--task1', type=str, default='definition',
    help='type of init rnn hidden'
)
parser.add_argument(
...
valid_dataset = []
valid_dataloader = []
for i in range(len(args["label_type"])):
    train_dataset.append(DefinitionModelingDataset(
        file=args["train_defs"],
        vocab_path=args["voc"],
        label=args["label_type"][i],
    ...
    valid_dataset.append(DefinitionModelingDataset(
        file=args["eval_defs"],
        vocab_path=args["voc"],
        label=args["label_type"][i],
        context_vocab_path=args["context_voc"],
    ...
    valid_dataloader.append(DataLoader(
        valid_dataset[i],
        batch_size=args["batch_size"],
        collate_fn=DefinitionModelingCollate,
        shuffle=True,
...
if args["use_input_attention"]:
    ...
    args["n_attn_tokens"] = len(train_dataset[-1].context_voc.token2id)
...
if args["use_ch"]:
    ...
    args["n_ch_tokens"] = len(train_dataset[-1].ch_voc.token2id)
    args["ch_maxlen"] = train_dataset[-1].ch_voc.token_maxlen + 2
...
args["vocab_size"] = len(train_dataset[-1].voc.token2id)
...
def train():
    ...
    for epoch in range(args["epochs"]):
        ...
        train_loss, train_ppl, task1_loss, task1_ppl, task2_loss, task2_ppl = train_epoch(
            ...
            args["ac_re"], args["alpha"], args["beta"]
        )
        ...
        if valid2_ppl < best_ppl:
            ...
            torch.save(model.state_dict(), args["exp_dir"] +
                       'hir_shared_%s_min_ppl.pkl' % (epoch + 1)
                       )
            improved_str = '*'
        else:
            improved_str = ''
        ...
        print(task1_msg.format(args["label_type"][0], valid1_loss, valid1_ppl) + "\n")
        print(task2_msg.format(args["label_type"][1], valid2_loss, valid2_ppl, improved_str) + "\n")
